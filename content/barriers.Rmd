...
The location data is available in three formats: 
...
* an [ESRI shapefile](https://en.wikipedia.org/wiki/Shapefile) that can be opened by GIS applications, and 
* a [`.geoJSON`](https://en.wikipedia.org/wiki/GeoJSON) file that can also be opened by GIS tools. 

These data are released under an [Open Data Commons Attribution License (ODC-By)](http://opendatacommons.org/licenses/by/1.0/), which is a permissive license that allows the data to be widely reused as long as attribution is provided. The data are available through [GitHub](https://github.com/chris-prener/barriers-data), and will be subject to regular updates. We also plan to release additional data about the barriers as we proceed with our project, so please check back often!
